...
$$\tilde{y}_i = \sum_{j = 1}^n w_j * \tilde{y}_i^j$$. The simplest weighting method would be to weight all models equally and set $w_j = \frac{1}{n}$ for all $j$. A random forest combines the ideas of model ensembles and decision trees in the following way given the parameters $n_{trees}$, $n_{sample}$, $n_{variables}$:
\begin{enumerate}
...
To still be able to asses the importance of the different variables a variety of methods. One importance measure that is often used and will also be included in the later sections of this report is the \textit{Scaled Permutation Importance}. To calculate this measures one takes the following steps:
\begin{enumerate}
