...
# function, that crates a correlation plot 
...
corr.barplot = function(numb.corr = 36) {
    if (numb.corr > ncol(numeric.data) - 1) {
        ...
        df$x.plotting = factor(df$x.plotting, levels = df[order(df$y.plotting, decreasing = TRUE), "x.plotting"])
...
# plotting the barplot to the RStudio pane
corr.barplot()
...
# printing the plots to the RStudio pane
